...
***There are `r nrow(dennys)` rows and `r ncol(dennys)` columns. Each row represents data for one denny's restaurant. The variables are:
`r names(dennys)`.***
 ...
 ***There are `r nrow(laquinta)` rows and `r ncol(laquinta)` columns. Each row represents data for one La Quinta Inn. The variables are:
`r names(laquinta)`.***
...
3.  Take a look at the websites that the data come from (linked above).
  ...
  ***La Quinta hotels are mostly located in the America's, but also have locations in other parts of the world. Denny's are only located in the US.***
...
***We can use the state variable to determine wether a location is in the US, specifically by comparing the state listed to a database of states in the US.***
...
***There are no Denny's locations outside of the US in our data.***
